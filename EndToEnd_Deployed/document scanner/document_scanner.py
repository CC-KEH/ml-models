...
def preprocessing(img):
    ...
    img_dial = cv2.dilate(img_canny,kernel,iterations=2) # Double Dilation; To Increase the width of the edges
    img_eroded = cv2.erode(img_dial,kernel,iterations=1) # Shrinking the edges little bit, also removing noise
    return img_eroded
...
def get_warp(img,biggest):
    ...
    cropped_img = img_output[20:img_output.shape[0]-20,20:img_output.shape[1]-20]
...
while True:
    ...
    if biggest.any():
        ...
        print("Bring something big in front of the camera.")
